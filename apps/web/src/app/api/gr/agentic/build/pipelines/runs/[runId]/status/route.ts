...
export async function POST(
    ...
    try {
        ...
        const idempotencyKey = request.headers.get('idempotency-key') || `pipeline-status-${runId}-${Date.now()}`

        const upstream = await backendPost(
            `/v1/agentic/build/pipelines/runs/${encodeURIComponent(runId)}/status`,
            body,
            idempotencyKey,
            request
        )
        ...
        return NextResponse.json(
            {
                error: 'agentic_pipeline_status_update_failed',
