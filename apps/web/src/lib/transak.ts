...
export function openTransakWidget(params: {
  ...
  // ── Close button (always works — not dependent on iframe state) ───────────
  ...
  // ── postMessage listener for Transak events ───────────────────────────────
  function onMessage(event: MessageEvent) {
    if (event.origin !== new URL(widgetUrl).origin) return
    const type: string = event.data?.event_id ?? event.data?.type ?? ''
    if (type === 'TRANSAK_ORDER_SUCCESSFUL') {
      params.onSuccess?.(event.data?.data as TransakOrderData)
      cleanup()
    } else if (type === 'TRANSAK_WIDGET_CLOSE') {
      cleanup()
    }
  }
